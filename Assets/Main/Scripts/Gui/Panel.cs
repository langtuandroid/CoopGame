...
namespace Main.Scripts.Gui
{
    public class Panel : PooledObject
    {
            ...
            get
            {
                ...
                switch (_hideAt)
                {
                    ...
                    case HideLocation.LOWER_EDGE:
                        hidden.y = prt.rect.height *
                                   (rt.anchorMin.y * (rt.pivot.y - 1.0f) - rt.anchorMax.y * rt.pivot.y) +
                                   (rt.pivot.y - 1.0f) * _hideScale.y * rt.rect.height;
                        break;
                    case HideLocation.UPPER_EDGE:
                        hidden.y = prt.rect.height *
                                   (1 + rt.anchorMin.y * (rt.pivot.y - 1.0f) - rt.anchorMax.y * rt.pivot.y) +
                                   (rt.pivot.y) * _hideScale.y * rt.rect.height;
                        break;
                    case HideLocation.LEFT_EDGE:
                        hidden.x =
                            prt.rect.width * (rt.anchorMin.x * (rt.pivot.x - 1.0f) - rt.anchorMax.x * rt.pivot.x) +
                            (rt.pivot.x - 1.0f) * _hideScale.x * rt.rect.width;
                        break;
                    case HideLocation.RIGHT_EDGE:
                        hidden.x = prt.rect.width *
                                   (1 + rt.anchorMin.x * (rt.pivot.x - 1.0f) - rt.anchorMax.x * rt.pivot.x) +
                                   (rt.pivot.x) * _hideScale.x * rt.rect.width;
                        break;
        ...
        private Vector2 MapAnchoredPosition(RectTransform from, RectTransform to)
        {
            ...
            Vector2 cto = new Vector2((from.anchorMin.x + from.anchorMax.x) * wto / 2,
                (from.anchorMin.y + from.anchorMax.y) * hto / 2);

            float wfrom = ((RectTransform) to.parent).rect.width;
            float hfrom = ((RectTransform) to.parent).rect.height;
            Vector2 cfrom = new Vector2((to.anchorMin.x + to.anchorMax.x) * wfrom / 2,
                (to.anchorMin.y + to.anchorMax.y) * hfrom / 2);
        ...
        private void Hide(bool immediately = false, Action then = null)
        {
            ...
            if (gameObject.activeInHierarchy)
                _coroutine = StartCoroutine(AnimateRect(immediately, _shownPos, hiddenPos, _shownScale, _hideScale,
                    _shownSize, hiddenSize, _showAlpha, _hideAlpha, Tween.GetEasingFunction(_hideEase),
                    onHidden));
            else
                AnimateRect(true, _shownPos, hiddenPos, _shownScale, _hideScale, _shownSize, hiddenSize, _showAlpha,
                    _hideAlpha, Tween.GetEasingFunction(_hideEase), onHidden);
        }
        ...
        private void Show(bool immediately = false, Action then = null)
        {
            ...
            if (_hideAlpha < 1.0f && _orgAlphas == null)
                RememberAlphas();
            _coroutine = StartCoroutine(AnimateRect(immediately, hiddenPos, _shownPos, _hideScale, _shownScale,
                hiddenSize, _shownSize, _hideAlpha, _showAlpha, Tween.GetEasingFunction(_showEase), () =>
                {
                    DidShow();
                    if (onDidShow != null)
                        onDidShow(this);
        ...
        protected IEnumerator AnimateRect(bool immediately, Vector2 from_position, Vector2 to_position,
            Vector2 from_scale, Vector2 to_scale, Vector2 from_size, Vector2 to_size, float from_alpha,
